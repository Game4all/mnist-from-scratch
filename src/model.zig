...
pub fn MNISTClassifier(comptime num_batches: usize) type {
    return struct {
        ...
        pub fn forward(self: *@This(), device: Device, input: *const Tensor(f32)) *Tensor(f32) {
            ...
            const c = self.layer_3.forward(device, b);
            return self.layer_4.forward(device, c);
        }

        pub fn backwards(self: *@This(), device: Device, loss_grad: *const Tensor(f32)) void {
            const D = self.layer_4.backwards(device, loss_grad);
            const C = self.layer_3.backwards(device, D);
            const B = self.layer_2.backwards(device, C);
            _ = self.layer_1.backwards(device, B);
        }
        ...
        pub fn step(self: *@This(), device: Device, ins: *const Tensor(f32), lr: f32) !void {
            ...
            brainz.ops.matMul(f32, device, &self.layer_3.grad, &layer3_inputs, &self.weight_grad_3);
            brainz.ops.matMul(f32, device, &self.layer_4.grad, &layer4_inputs, &self.weight_grad_4);
            ...
            brainz.ops.reduce(f32, device, .Sum, &self.weight_grad_3, 0, &self.weight_grad_3_f);
            brainz.ops.reduce(f32, device, .Sum, &self.weight_grad_4, 0, &self.weight_grad_4_f);
            ...
            brainz.ops.reduce(f32, device, .Sum, &self.layer_3.grad, 0, &self.bias_grad_3);
            brainz.ops.reduce(f32, device, .Sum, &self.layer_4.grad, 0, &self.bias_grad_4);
            ...
            brainz.ops.sub(f32, device, &self.layer_1.weights, &self.weight_grad_1_f, &self.layer_1.weights, .{ .alpha = alpha });
            brainz.ops.sub(f32, device, &self.layer_1.biases, &self.bias_grad_1, &self.layer_1.biases, .{ .alpha = alpha });

            brainz.ops.sub(f32, device, &self.layer_2.weights, &self.weight_grad_2_f, &self.layer_2.weights, .{ .alpha = alpha });
            brainz.ops.sub(f32, device, &self.layer_2.biases, &self.bias_grad_2, &self.layer_2.biases, .{ .alpha = alpha });

            brainz.ops.sub(f32, device, &self.layer_3.weights, &self.weight_grad_3_f, &self.layer_3.weights, .{ .alpha = alpha });
            brainz.ops.sub(f32, device, &self.layer_3.biases, &self.bias_grad_3, &self.layer_3.biases, .{ .alpha = alpha });

            brainz.ops.sub(f32, device, &self.layer_4.weights, &self.weight_grad_4_f, &self.layer_4.weights, .{ .alpha = alpha });
            brainz.ops.sub(f32, device, &self.layer_4.biases, &self.bias_grad_4, &self.layer_4.biases, .{ .alpha = alpha });
